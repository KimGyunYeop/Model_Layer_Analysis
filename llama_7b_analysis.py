...
data = datasets.load_dataset("google/boolq")
print(data)
...
if prompt_data_num > 0:
    ...
    print(prompt_data)
    prompt_string = " ".join([prompt_format.format(prompt_data["passage"][i] ,prompt_data["question"][i], prompt_data["answer"][i]) for i in range(3)])
    print(prompt_string)
else:
    prompt_string = ""
...
for test_idx in range(test_num):
    ...
    # print(input_data)
    out = model.generate(**input_data, max_new_tokens=300, pad_token_id=tokenizer.eos_token_id)
    # print(out)
...
print("acc = ",correct_counter/float(test_num))
